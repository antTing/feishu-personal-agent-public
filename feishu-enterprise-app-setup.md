...
Aily（可选） -> 执行群 -> 飞书企业自建应用机器人
                            ...
                            -> cc-connect 原生 Agent（Codex / Claude Code / Cursor ACP）
```
...
配对监听器不启动本地执行 Agent，不处理普通任务，也不保存消息正文。中间状态和最终配置只在被忽略的 `runtime/` 中以 `700/600` 权限保存；授权链接和随机码默认只显示在自动打开的本机浏览器中，不进入 Agent 输出，凭据和身份 ID 始终不显示。
